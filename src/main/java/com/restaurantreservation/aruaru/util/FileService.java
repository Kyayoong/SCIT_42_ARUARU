...
public class FileService {
	 ...
	 * @return 저장된 파일명
	 */
	public static String saveFile(List<MultipartFile> mfile, String uploadPath) {
		//업로드된 파일이 없거나 크기가 0이면 저장하지 않고 null을 리턴
		if (mfile == null || mfile.isEmpty() || mfile.size() == 0) {
			return null;
		}
		...
		for(int i = 0; i<mfile.size(); ++i) {
			//원본 파일명
			String originalFilename = mfile.get(i).getOriginalFilename();
			...
			//파일 저장
			try {
				mfile.get(i).transferTo(serverFile);
			} catch (Exception e) {
				savedFilename = null;
				e.printStackTrace();
			}
